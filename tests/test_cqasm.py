...
from pennylane_quantuminspire.cqasm import dumps, dumps_new
...
def test_dumps_new(mocker: MockerFixture, QI_backend: QIBackend) -> None:
    device = QIDevice(backend=QI_backend)
    ...
    assert dumps_new(quantum_function) == expected_cqasm
